        ...
        <div className="container mx-auto px-4">
          {filteredTenders.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {filteredTenders.slice().reverse().map((tender:any) => (
                <div
                  key={tender.id}
                  className="bg-white rounded-2xl p-6 shadow-sm hover:shadow-lg transition-all duration-300 border-2 border-cml-green"
                >

                  <div className="flex place-items-baseline justify-between mb-4 gap-3 md:gap-10">
                    {/* Title */}
                    <h3 className=" text-xl md:text-2xl font-bold text-cml-black leading-tight">
                      {tender.title}
                    </h3>
                    {/* Status */}
                    {isActive(tender.status) ? (
                      ...
                      <span className="bg-red-100 min-w-[120px] md:w-[180px] text-red-600 px-4 py-2 rounded-full text-sm font-bold">
                        NOT ACTIVE
                      </span>
                    )}
                    ...
                    <ul className="space-y-2">
                      {tender.requirements.map((req, index) => (
                        <li key={index} className="text-sm text-gray-700 flex items-start">
                          <span className="w-1 h-1 bg-cml-black rounded-full mt-2 mr-3 flex-shrink-0"></span>
                          {req}
                        </li>
                      ))}
                    </ul>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div className="space-y-2 mb-6 text-sm">
                      <div>
                        <span className="font-bold text-cml-black">TENDER ID: {tender.tender_id}</span>

                      </div>
                      <div>
                        <span className="font-bold text-cml-black">LAST DATE: {formatDate(tender.last_date)}</span>

                      </div>
                      <div>
                        <span className="font-bold text-cml-black">ESTIMATED VALUE: {formatCurrency(tender.estimated_value)}</span>
                    ...
                    <button
                      ...
                      className={`font-bold py-3 px-6 rounded-full transition-colors duration-300 flex items-center justify-center gap-2 ${isActive(tender.status)
